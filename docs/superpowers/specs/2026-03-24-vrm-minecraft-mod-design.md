...
│   └── src/main/kotlin/com/github/narazaka/vrmmod/
│       ├── vrm/               # VRMパーサー・モデル定義
│       ├── render/            # レンダリングエンジン
...
将来的にVRM 0.xパーサーを追加実装し、同じ `VrmModel` に変換するアダプターを書く。内部表現を共通にすることで、レンダリング以降は0.x/1.0を意識しない。
...
class PlayerRendererMixin {
    ...
    fun onRender(player: AbstractClientPlayer, ..., ci: CallbackInfo) {
        val vrmState = VrmPlayerManager.get(player) ?: return
        VrmRenderer.render(vrmState, poseStack, bufferSource, light)
        ci.cancel()
    }
}
```
...
パフォーマンス問題が生じた場合、SSBO + Compute ShaderによるGPUスキニングへの移行を検討。
...
| フェーズ | 内容 | 成果物 |
|---|---|---|
| 1 | プロジェクト構築 + VRMパーサー | .vrmファイルが読めること |
...
- **JglTFのメンテナンス状況**: 更新頻度が低い場合、glTFパーサーの自前実装やAssimp（LWJGL経由）への切り替えが必要になる可能性
- **Minecraft バージョンアップ追従**: レンダリングAPIの変更への対応工数。Architecturyである程度は吸収される
- **CPUスキニングのパフォーマンス**: 高ポリゴンモデル × 多人数でFPS低下。ポリゴン上限設定で緩和
