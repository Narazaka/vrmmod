...
- アニメーションの左右反転（mirror）オプションによる利き手対応
...
[VRoid Hub](https://hub.vroid.com/) 上のモデルを直接読み込んで使用できます。設定画面の「モデルソース」を `VROID_HUB` に切り替え、OAuth 認証を行うと、自分のモデルやハートしたモデルから選択できます。

### マルチプレイ同期

サーバーにもこの MOD が導入されている場合、各プレイヤーの VRoid Hub モデルが他のプレイヤーにも表示されます。

- **仕組み**: プレイヤーがモデルを設定すると、VRoid Hub のマルチプレイ用ダウンロードライセンスを取得してサーバー経由で共有します。受信側は自分の VRoid Hub 認証でモデルをダウンロードします
- **要件**: 他プレイヤーのモデルを表示するには、受信側も VRoid Hub にログインしている必要があります
- **モデルの公開設定**: VRoid Hub 側で「モデル登録者以外の利用」が許可されていないモデルは、他プレイヤーには表示されません
- **MOD 未導入サーバー**: サーバーに MOD がない場合でも、従来通り自分のモデルだけが表示されます（エラーは発生しません）
- **同期される設定**: モデル ID、アバタースケール、法線モード（NormalMode）が他プレイヤーに共有されます
- **アニメーション**: 各クライアントが Minecraft の標準的なプレイヤー状態から独立計算するため、アニメーション同期用の追加パケットはありません
