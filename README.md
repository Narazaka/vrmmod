...
Minecraft のプレイヤーモデルを VRM 1.0 アバターに置き換える MOD です。

## 機能

### VRM アバター表示

ローカルの `.vrm` ファイル（VRM 1.0 形式）をプレイヤーモデルとして描画します。OPAQUE / BLEND（半透明）のアルファモードに対応しています。デフォルトでは法線を均一化した Unlit 風のシェーディングで、Minecraft の光源に左右されないフラットな見た目になります。
...
[VRoid Studio](https://vroid.com/studio) などで作成した VRM 1.0 形式のモデルを使用できます。`.vrm` ファイルの絶対パスを設定の `localModelPath` に指定してください。
